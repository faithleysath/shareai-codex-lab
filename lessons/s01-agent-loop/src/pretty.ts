...
export function logLoopPrimer(): void {
  printBlock(
    ...
    [
      ...
      "3. Append the tool output as a new `tool` message.",
      "4. Ask the model again with the expanded message history.",
      "5. Stop when the assistant returns plain text instead of another tool call.",
    ].join("\n"),
    ANSI.cyan,
  );
}
