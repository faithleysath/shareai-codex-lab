...
export function formatToolResultPreview(result: unknown): string {
  if (
    ...
    return [
      ...
      `completed_count: ${typed.completed_count}`,
      `in_progress_count: ${typed.in_progress_count}`,
      "todo:",
      indent(typed.rendered),
      typed.note ?? "",
    ]
      .filter(Boolean)
      .join("\n");
...
export function logLoopPrimer(): void {
  printBlock(
    ...
    [
      ...
      "2. On a multi-step task, the agent should first externalize a plan with todo_write.",
      "3. If the assistant returns tool_calls, run the local TypeScript tool ourselves.",
      "4. Resolve each tool name through a dispatch map like handlers[toolName].",
      "5. Append the tool output as a new `tool` message.",
      "6. Ask the model again with the expanded message history.",
      "7. If the plan goes stale for too many rounds, inject a reminder.",
      "8. Stop when the assistant returns plain text instead of another tool call.",
    ].join("\n"),
    ANSI.cyan,
  );
}
...
export function logTodoReminderInjected(roundsSinceTodo: number): void {
  console.log(
    paint(
      `\n[todo-reminder] No todo update for ${roundsSinceTodo} rounds. Injecting a reminder message before the next model call.`,
      ANSI.dim,
      ANSI.gray,
    ),
  );
