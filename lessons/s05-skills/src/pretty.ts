...
export function formatToolResultPreview(result: unknown): string {
  if (
    ...
    typeof (result as { skill_content?: unknown }).skill_content === "string"
  ) {
    ...
    return [
      ...
      `title: ${typed.title ?? "(none)"}`,
      `summary: ${typed.summary ?? "(none)"}`,
      `trigger_hints: ${(typed.trigger_hints ?? []).join(", ")}`,
      "skill_content:",
      indent(typed.skill_content),
      typed.note ?? "",
    ]
      .filter(Boolean)
      .join("\n");
...
export function logBanner(info: {
  ...
  console.log(`${paint("promptLabel", ANSI.bold)} ${info.promptLabel}`);
...
export function logLoopPrimer(): void {
  printBlock(
    "Loop Primer",
    [
      "1. Send the system prompt, user prompt, and the load_skill schema to the model.",
      "2. The system prompt only contains short skill summaries, not the full skill text.",
      "3. If the model decides a skill is relevant, it calls load_skill with the skill name.",
      "4. Local TypeScript code returns the full skill content as a tool result.",
      "5. We append that tool result back into message history, which acts like a second-layer prompt injection.",
      "6. The model answers using the freshly loaded skill, or answers directly when no skill is needed.",
    ].join("\n"),
    ANSI.cyan,
  );
}

export function logSkillIndex(skills: SkillDefinition[]): void {
  printBlock(
    "Skill Index (Summary Only)",
    skills
      .map((skill) =>
        [
